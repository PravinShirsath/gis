...
<h5 style="margin-left:20px;"><a href="<?php echo base_url();?>index.php/PumpsController/create"><button>Add Distributor</button></a>
	...
	<a href="<?php echo base_url();?>index.php/PumpsController/deactivatelist"><button style="background-color:RED; COLOR:WHITE">Deactive Distributors list</button></a>
                    ...
                    <th>Distributor Name -
					  <font color="red" style="text-align:right;"><?php 
									...
									foreach($pumps as $de){
										...
										echo ' Deactive '.number_format($total1);
								?></font>
					  </th>                   
                    <th>Oil Company</th>
                    <th>City</th>
				...
				?>
                  <tr>
                    ...
                    <td><?php echo $pump->mobileNo;?></td>
					 <td><button title="Activate" class="btn btn-primary" name="delete" id="delete" value="<?php echo $pump->id; ?>" onclick="activatepump(this.value);"> Activate</button></td>  
                  </tr>
				 <?php } } ?>
                </tbody>
              </table>
...
$('#regions').bind("change",function () {
 ...
 <script type="text/javascript">
	...
	<script>
function activatepump(value)
{
if(confirm("Sure to activate record !!!") == true)
{
$.ajax({
type: 'post',
url: '<?php echo base_url();?>index.php/PumpsController/activatepump',
data: 'post_id=' + value,
